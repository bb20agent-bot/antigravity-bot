...
// test_logic.ts - Simulation of TON Contract Logic
...
function distributeRewards(buyerAddr: Address, amount: number) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (remaining >= reward) {
            ...
            remaining -= reward;
            console.log(`  Level ${i + 1}: ${uplineAddr} gets ${reward} (Rate ${rates[i] * 100}%)`);
        }
    ...
    console.log(`  Treasury (Admin) gets remaining: ${remaining}`);
}
...
Object.values(USERS).forEach(u => {
    console.log(`${u.address}: ${u.balance}`);
});
