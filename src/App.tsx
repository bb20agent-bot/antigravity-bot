  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#050505] text-white max-w-md mx-auto relative overflow-hidden pb-32 border-x border-white/15">
      ...
      <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-[360px] bg-black/40 backdrop-blur-3xl border border-white/10 rounded-full py-4.5 px-8 z-50 flex justify-between items-center shadow-2xl">
        <NavItem icon={<LayoutDashboard size={22} />} active={activeTab === NavTab.HOME} onClick={() => setActiveTab(NavTab.HOME)} label="Home Dashboard" />
        <NavItem icon={<Video size={22} />} active={activeTab === NavTab.LIVE} onClick={() => setActiveTab(NavTab.LIVE)} label="Live Coaching" />
        <NavItem icon={<Gamepad2 size={22} />} active={activeTab === NavTab.MY_OFFICE} onClick={() => setActiveTab(NavTab.MY_OFFICE)} label="My Office" />
        <NavItem icon={<ShoppingBag size={22} />} active={activeTab === NavTab.MARKETPLACE} onClick={() => setActiveTab(NavTab.MARKETPLACE)} label="Marketplace" />
        <NavItem icon={<Crown size={22} />} active={activeTab === NavTab.INFO} onClick={() => setActiveTab(NavTab.INFO)} label="Fandom Subscription Info" />
      </nav>
...
const NavItem = ({ icon, active, onClick, className, label }: { icon: React.ReactNode, active: boolean, onClick: () => void, className?: string, label: string }) => (
  <button onClick={onClick} aria-label={label} title={label} className={`p-2 transition-all relative focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0088cc] rounded-lg ${className} ${active ? 'scale-125' : 'text-gray-600'}`}>
