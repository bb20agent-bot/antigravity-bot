    ...
    return (
      <div className="px-5 py-4 space-y-6 pb-24 animate-in fade-in duration-500">
        <div className="flex items-center gap-3 mb-4">
          <button onClick={() => setShowAcademy(false)} aria-label="Close Academy" className="bg-white/10 p-2 rounded-xl text-white hover:bg-white/20"><AlertTriangle className="rotate-90 hidden" /><ArrowUpRight className="rotate-[225deg]" size={20}/></button>
  ...
  return (
    <div className="relative">
      <div className={`flex items-center gap-3 p-3 rounded-2xl border transition-all ${user.status === 'Active' ? 'bg-white/5 border-white/10' : 'bg-black/40 border-white/5 opacity-50'}`}>
        ...
        {hasChildren && <button onClick={() => setIsExpanded(!isExpanded)} aria-label={isExpanded ? "Collapse" : "Expand"} className="p-1 text-gray-500">{isExpanded ? <ChevronDown size={14} /> : <ArrowRight size={14} />}</button>}
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#050505] text-white max-w-md mx-auto relative overflow-hidden pb-32 border-x border-white/15">
      ...
      <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-[360px] bg-black/40 backdrop-blur-3xl border border-white/10 rounded-full py-4.5 px-8 z-50 flex justify-between items-center shadow-2xl">
        <NavItem icon={<LayoutDashboard size={22} />} active={activeTab === NavTab.HOME} onClick={() => setActiveTab(NavTab.HOME)} aria-label="Dashboard" />
        <NavItem icon={<Video size={22} />} active={activeTab === NavTab.LIVE} onClick={() => setActiveTab(NavTab.LIVE)} aria-label="Live Stream" />
        <NavItem icon={<Gamepad2 size={22} />} active={activeTab === NavTab.MY_OFFICE} onClick={() => setActiveTab(NavTab.MY_OFFICE)} aria-label="My Office" />
        <NavItem icon={<ShoppingBag size={22} />} active={activeTab === NavTab.MARKETPLACE} onClick={() => setActiveTab(NavTab.MARKETPLACE)} aria-label="Marketplace" />
        <NavItem icon={<Crown size={22} />} active={activeTab === NavTab.INFO} onClick={() => setActiveTab(NavTab.INFO)} aria-label="Fandom Plans" />
      </nav>
