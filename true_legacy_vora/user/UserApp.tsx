                    ...
                    {points > 0 && (
                        <button
                            onClick={async () => {
                                ...
                                let telegramId = (window as any).Telegram?.WebApp?.initDataUnsafe?.user?.id?.toString() || "dev_user_123";
                                try {
                                    // [SECURITY] Vault ClaimReward 가스 수수료 (0.08 TON = 80,000,000 nanoTON) 전송 강제
                                    const transaction = {
                                        validUntil: Math.floor(Date.now() / 1000) + 360,
                                        messages: [
                                            {
                                                address: "0QDQv-uJ7vO8Z1c_tQp3oO1eA9tFtz37yOov__t587hS_Yn4", // Vault Address (Placeholder)
                                                amount: "80000000", // 0.08 TON
                                            }
                                        ]
                                    };
                                    ...
                                    alert("가스비 트랜잭션 전송이 취소되었거나 실패했습니다.");
                        ...
                        >Withdraw VORA (Fee: 0.08 TON)</button>
                    )}
                        ...
                        <div className="px-2">
                            <div className="bg-[#111] border border-white/10 rounded-2xl p-4 flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xs font-mono text-cyan-400">https://t.me/Crew_Meetup_Bot/app?startapp={(window as any).Telegram?.WebApp?.initDataUnsafe?.user?.id || 'TESTUSER'}</p>
                                </div>
